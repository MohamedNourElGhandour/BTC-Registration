...
namespace Eng1
{
    public partial class frmUpdateStudentByCourse : Form
    {
        public frmUpdateStudentByCourse()
        {
            InitializeComponent();
        }
        bool load=false ;
        ...
        bool load2 = false;
        int classvalue;
        private void frmUpdateStudentByCourse_Load(object sender, EventArgs e)
        {
        ...
        private void comboCourseSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (load==true )
            {
                using (OleDbConnection con = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data source=English_DB.mdb"))
                {
                    using (OleDbCommand com = new OleDbCommand("select group_ID from Groups where CourseID=" + comboCourseSearch.SelectedValue + " ", con))
                    {
                        OleDbDataAdapter da = new OleDbDataAdapter(com);
                        DataTable dt = new DataTable();
                        da.Fill(dt);
                        comboGroupSearch.DataSource = dt;
                        comboGroupSearch.ValueMember = dt.Columns[0].ToString();
                        comboGroupSearch.DisplayMember = dt.Columns[0].ToString();
        ...
        private void frmUpdateStudentByCourse_Shown(object sender, EventArgs e)
        {
            load = true;
        }

        private void comboGroupSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (load1 == true && load == true)
                {
                    using (OleDbConnection con = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data source=English_DB.mdb"))
                    {
                        using (OleDbCommand com = new OleDbCommand("select Stud_ID,Student_Name_Ar from St where group_ID=" + comboGroupSearch.SelectedValue + "", con))
                        {
                            OleDbDataAdapter adap = new OleDbDataAdapter(com);
                            DataTable dt = new DataTable();
                            adap.Fill(dt);
        ...
        private void comboGroupSearch_DropDown(object sender, EventArgs e)
        {
            load1 = true;
        }
